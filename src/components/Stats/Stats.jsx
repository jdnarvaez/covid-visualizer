  ...
  if (activeMetric && timeseries) {
    switch (activeMetric) {
      ...
      case 'HospitalBeds':
        ...
        yAccessor = (d) => d.hospitalBeds && d.hospitalBeds.currentUsageTotal;
        ...
        {summary && <React.Fragment>
          ...
          <div className="stats-content">
            ...
            <div className="metrics" data-tour="hospital-metrics">
              {summary.actuals.hospitalBeds && <Metric icon={<GiBed />} isSelected={'HospitalBeds' === activeMetric} property="HospitalBeds" label="Hospital Beds" value={`${Number.parseInt(summary.actuals.hospitalBeds.capacity).toLocaleString()} (${Number.parseFloat((summary.actuals.hospitalBeds.currentUsageTotal / summary.actuals.hospitalBeds.capacity) * 100).toFixed(0)}% Full, ${Number.parseFloat((summary.actuals.hospitalBeds.currentUsageCovid / summary.actuals.hospitalBeds.capacity) * 100).toFixed(0)}% COVID, ${Number.parseFloat(summary.actuals.hospitalBeds.typicalUsageRate * 100).toFixed(0)}% Typical)`} activateMetric={activateMetric} activeMetric={activeMetric} />}
              {summary.actuals.icuBeds && <Metric icon={<GiPersonInBed />} isSelected={'ICUBeds' === activeMetric} property="ICUBeds" label="ICU Beds" riskLevel={summary.riskLevels.icuCapacityRatio} value={`${Number.parseInt(summary.actuals.icuBeds.capacity).toLocaleString()} (${Number.parseFloat((summary.actuals.icuBeds.currentUsageTotal / summary.actuals.icuBeds.capacity) * 100).toFixed(0)}% Full, ${Number.parseFloat((summary.actuals.icuBeds.currentUsageCovid / summary.actuals.icuBeds.capacity) * 100).toFixed(0)}% COVID, ${Number.parseFloat(summary.actuals.icuBeds.typicalUsageRate * 100).toFixed(0)}% Typical)`} activateMetric={activateMetric} activeMetric={activeMetric} />}
            </div>
